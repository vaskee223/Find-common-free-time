...
def compare_timelines(timelines:list):
    ...
    if(len(timelines)==0):
        ...
        print(find_free_times(timelines[0].timeline["schedule"], timelines[0].timeline["start"]))
        return
    for timeline in timelines:
        if timeline.timeline["start"]<start:
            min = timeline.timeline["start"]
        if timeline.timeline["end"]>end:
            max = timeline.timeline["end"]
    alligned = normalize_timeline(timelines[0].timeline["schedule"], timelines[0].timeline["start"], timelines[0].timeline["end"])
    for i in range(1, len(timelines)+1):
        alligned &= normalize_timeline(timelines[i].timeline["schedule"], alligned["start"], alligned["end"])
...
class Timeline:
    ...
    def set_busy_time(self):
        ...
        print(match)
        print(validate_times(match))
        if match is None or len(match) != 4 or not validate_times(match) or self.timeline["start"] > int(match[0]) or self.timeline["end"] < int(match[2]):
...
t1 = Timeline(8, 16)
t1.set_free_time("12 00 16 00")
t2 = Timeline(8, 16)
t2.set_free_time("13 00 15 00")

compare_timelines([t1, t2])
